...
class Dataset:
    ...
    def initialize(self):
        ...
        self.targets_mean = meta.mean(axis=0)[-1]
        self.targets_std  = meta.std()[-1]
...
class DataLoaderIterator:
    def __init__(self, dataloader: DataLoader):
        ...
        self.features = dataloader.dataset.features
        self.targets  = dataloader.dataset.targets
    ...
    def __next__(self):
        ...
        batch_size = min(self.batch_size, len(self.dataset)-self.cursor)

        batch_features = self.features[self.cursor:self.cursor + batch_size]
        batch_targets  = self.targets[self.cursor:self.cursor + batch_size]
            
        features = np.vstack(batch_features)
        targets  = np.vstack(batch_targets)
...
if __name__ == '__main__':
    file = '/Users/azen/Desktop/myAir/Work/Workspace/Others/DeepLearning-Tutorial/1.Prerequisites/1.4.LinearRegression/datasets/BostonHousePrices.csv'
    num_features = 13
    batch_size = 16
